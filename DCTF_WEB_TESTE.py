...
DESCRICAO_DCTFWEB_ALVO = "DCTFWeb – Declaração de Débitos e Créditos Tributários Federais"
...
def extrair_mes_ano_referencia(periodo_texto):
    ...
    # Ex.: "ano-calendario 2025" -> vencimento cai em janeiro do ano seguinte (base)
...
def calcular_vencimento(periodo, prazo, meses_dinamica=1):
...
def obter_dados_dctfweb_por_excel(caminho_excel=ARQUIVO_EXCEL):
    """
    Retorna dict com:
    {
    ...
    descricao_alvo_norm = normalizar_texto(DESCRICAO_DCTFWEB_ALVO)
    ...
    for _, row in df.iterrows():
        ...
        # match pela descrição completa (mais específico)
        if descricao_alvo_norm not in desc_norm:
            continue

        vencimento = calcular_vencimento(periodo, prazo, meses_dinamica=1)  # DCTFWeb = +1 mês
        if not vencimento:
            continue
    ...
    # Mantém o vencimento mais próximo (mesma ideia do seu script)
    candidatos.sort(key=lambda x: x["vencimento"])
    return candidatos[0]


def obter_vencimento_dctfweb_por_excel(caminho_excel=ARQUIVO_EXCEL):
    """
    Retorna somente o vencimento (datetime.date) ou None.
    Útil para importar no script legado.
    """
    dado = obter_dados_dctfweb_por_excel(caminho_excel)
    return dado["vencimento"] if dado else None


if __name__ == "__main__":
    dado = obter_dados_dctfweb_por_excel(ARQUIVO_EXCEL)
    ...
    if not dado:
        ...
        print("Vencimento DCTFWeb:", dado["vencimento"])
