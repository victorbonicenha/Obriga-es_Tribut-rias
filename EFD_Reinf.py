...
def eh_efd_reinf(descricao):
    """
    Match flexível para EFD-Reinf.
    Aceita variações com hífen, ponto, espaço e acentuação.
    """
    desc_norm = normalizar_texto(descricao)
    desc_compact = re.sub(r"[\s\-\.\u2013\u2014]+", "", desc_norm)
...
def obter_dados_efd_reinf_por_excel(caminho_excel=ARQUIVO_EXCEL):
    """
    Retorna dict com:
    {
    ...
    for _, row in df.iterrows():
        ...
        if not eh_efd_reinf(descricao):
            continue

        vencimento = calcular_vencimento(periodo, prazo, meses_dinamica=1)  # EFD-Reinf = +1 mês
        if not vencimento:
            continue
...
def obter_vencimento_efd_reinf_por_excel(caminho_excel=ARQUIVO_EXCEL):
    """
    Retorna somente o vencimento (datetime.date) ou None.
    Útil para importar no script da Agenda.
    """
    dado = obter_dados_efd_reinf_por_excel(caminho_excel)
    return dado["vencimento"] if dado else None


if __name__ == "__main__":
    dado = obter_dados_efd_reinf_por_excel(ARQUIVO_EXCEL)
    ...
    if not dado:
        ...
        print("Vencimento EFD-Reinf:", dado["vencimento"])
